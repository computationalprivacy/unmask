...
valid_prefixes_db, valid_prefixes_query = [], []
for identity in valid_identities:
    ...
    movies_chosen = rng.choice(movies_w_valid_results, size=2, replace = False)
    valid_prefixes_db.append(identity + '_' + movies_chosen[0])
    valid_prefixes_query.append(identity + '_' + movies_chosen[1])
...
for query_image in tqdm(list(query_dict_to_use.keys())):
    ...
    ground_truth_label = os.path.basename(query_image).split('.')[0][:-2] 
